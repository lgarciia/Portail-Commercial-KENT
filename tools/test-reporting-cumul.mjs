...
try {
  ...
  await page.locator('[data-cumul-projection] [data-cumul-seller]').first().click();
  assert.equal(await page.locator('[data-cumul-projection] .cumul-month-table tbody tr').count(), 12);
  ...
  if (xlsxBundle) {
    ...
    assert.equal(exported.sheets.Mensuel.length, 1 + 142 * 12);
    assert.equal(exported.sheets.Mensuel[7][6], null, 'Future actuals excluded in export');
  ...
  for (const width of [1440, 1280, 900, 390]) {
    ...
    if (width >= 1280) assert.ok(await page.locator('[data-cumul-ranking] .cumul-table-wrap').evaluate(el => el.scrollWidth <= el.clientWidth + 1), 'Full table visible on desktop');
  ...
  console.log(`PASS browser: hierarchy, complete monthly details, filters without requests, Top100/all, XSS escaping, desktop/mobile, partial error handling, request races. Screenshots: ${artifacts}`);
