    ...
    function cacheDom() {
      ...
      dom.clientNumeroClient = document.getElementById("clientNumeroClient");
    ...
    function clearClientContext() {
      ...
      dom.clientNumeroClient.textContent = "â€”";
    ...
    function updateClientHeader() {
      ...
      dom.clientNumeroClient.textContent = client.id ? String(client.id) : "â€”";
    ...
    function getFilteredClients() {
      ...
      return state.clients.filter(client =>
        [client.id, client.nom, client.numero_compte, client.telephone, client.adresse]
    ...
    function renderTimelineCard(visite) {
      ...
      const linesHtml = commandes.map(commande => {
        const produit = getProduitDisplay(commande.produit_id);
    ...
    function renderMatrixView() {
      ...
      if (!state.visites.length) {
        dom.visitesTableBody.innerHTML = `
          <tr>
            <td colspan="${produitsDynamiques.length + 4}">
    ...
    function buildProductDatalistOptions() {
      const pool = state.produitsActifs.length ? state.produitsActifs : state.produits;
    ...
    async function saveVisit(event) {
      ...
      if (!commandes.length) return showToast("Ajoute au moins un produit valide avec quantitÃ© ou stock.", "warning");

      const totalCommande = commandes.reduce((sum, commande) => {
      ...
      try {
        ...
        if (state.activeVisitId) {
          ...
          const { error: updateVisiteError } = await supabaseClient
            .from("visites")
            .update({ date_visite: dateVisite, note, total_commande: totalCommande })
            .eq("id", visiteId);
          ...
          const { data: visiteData, error: visiteError } = await supabaseClient
            .from("visites")
            .insert({ client_id: clientId, date_visite: dateVisite, note, total_commande: totalCommande })
            .select()
            .single();
    ...
    function attachEvents() {
      ...
      dom.addProductRowBtn.addEventListener("click", () => addProductRow());
      dom.visitForm.addEventListener("submit", saveVisit);
      dom.popupClient.addEventListener("change", refreshAllPopupPrices);
    ...
    async function initApp() {
      ...
      try {
        ...
        dom.produitsDatalist.innerHTML = buildProductDatalistOptions();
