(function () {
  ...
  var REPORT_SOURCES = [
    {
      ...
      label: "Auto",
      visitsTable: "visites",
      commandesTable: "visite_commandes",
      clientsTable: "clients",
      produitsTable: "produits",
      scopedTables: ["visites", "clients"]
    },
    {
      key: "industrie",
      label: "Industrie",
      visitsTable: "industrie_visites",
      commandesTable: "industrie_visite_commandes",
      clientsTable: "industrie_clients",
      produitsTable: "industrie_produits",
      scopedTables: ["industrie_visites", "industrie_clients"]
    }
  ];
  ...
  function shouldScopeTable(tableName) {
    return REPORT_SOURCES.some(function (source) {
      return source.scopedTables.includes(tableName);
    });
  }
  ...
  async function fetchRapportVisitesByDate(reportDate) {
    ...
    var batches = await Promise.all(
      REPORT_SOURCES.map(function (source) {
        return fetchRapportSourceVisites(reportDate, source);
      })
    );

    return batches
      .reduce(function (all, batch) {
        return all.concat(batch);
      }, [])
      .sort(function (a, b) {
        return (
          String(a.date_visite || "").localeCompare(String(b.date_visite || "")) ||
          String(a.source_label || "").localeCompare(String(b.source_label || "")) ||
          String((a.client && a.client.nom) || "").localeCompare(String((b.client && b.client.nom) || ""))
        );
      });
  }

  async function fetchRapportSourceVisites(reportDate, source) {
    ...
    if (visiteIds.length) {
      try {
        ...
        console.warn("Colonne demo_effectuee indisponible, rapport " + source.label + " chargé sans démos:", error.message);
        commandes = await fetchAllRapportRows(
          source.commandesTable,
    ...
    if (clientIds.length) {
      try {
        ...
        console.warn("Colonne taille_client indisponible, rapport " + source.label + " chargé avec taille S par défaut:", error.message);
        clients = (await fetchAllRapportRows(
          source.clientsTable,
    ...
    return visites.map(function (visite) {
      var rawClient = clientsById.get(String(visite.client_id)) || null;
      var scopedClientId = source.key + ":" + String(visite.client_id || "");
      var client = rawClient
        ? Object.assign({}, rawClient, {
            id: source.key + ":" + String(rawClient.id),
      ...
      return {
        id: source.key + ":" + String(visite.id),
        source_key: source.key,
        source_label: source.label,
        source_visite_id: visite.id,
        client_id: visite.client_id ? scopedClientId : "",
        ...
        commandes: (commandesByVisiteId.get(String(visite.id)) || []).map(function (cmd) {
          var rawProduit = produitsById.get(String(cmd.produit_id)) || null;
          var produit = rawProduit
            ? Object.assign({}, rawProduit, {
                id: source.key + ":" + String(rawProduit.id),
                source_key: source.key,
                source_label: source.label
              })
            : null;
          ...
          return {
            id: source.key + ":" + String(cmd.id),
            source_key: source.key,
            source_label: source.label,
            source_commande_id: cmd.id,
            produit_id: source.key + ":" + String(cmd.produit_id || ""),
  ...
  function calculateReportData(visites) {
    ...
    visites.forEach(function (visite) {
      ...
      var sourceLabel = visite.source_label || (visite.client && visite.client.source_label) || "Commerce";
      var clientId =
      ...
      clientEntry.sourceLabels.add(sourceLabel);
      clientEntry.sourceLabel = Array.from(clientEntry.sourceLabels).join(" + ");
      ...
      (visite.commandes || []).forEach(function (cmd) {
        ...
        if (!clientEntry.productsMap.has(String(productKey))) {
          clientEntry.productsMap.set(String(productKey), {
            ...
            sourceLabel: cmd.source_label || sourceLabel,
            sourceLabels: new Set([cmd.source_label || sourceLabel]),
            lines: 0,
            quantity: 0,
            stock: 0,
            ca: 0,
        ...
        clientProduct.sourceLabels.add(cmd.source_label || sourceLabel);
        clientProduct.sourceLabel = Array.from(clientProduct.sourceLabels).join(" + ");
        ...
        globalProduct.sourceLabels.add(cmd.source_label || sourceLabel);
        globalProduct.sourceLabel = Array.from(globalProduct.sourceLabels).join(" + ");
  ...
  function renderRankingRows(items, type) {
    if (!items.length) {
      return '<div class="analysis-item">Aucune donnée disponible pour cette date.</div>';
    }
        ...
        var meta =
          type === "client"
            ? "Compte : " +
              ...
              " · Univers : " +
              escapeHtml(item.sourceLabel || "Commerce") +
              " · Visites : " +
    ...
    if (executiveNode) {
      executiveNode.innerHTML =
        (stats.executivePoints || [])
          ...
          .join("") || '<div class="analysis-item">Aucune synthèse disponible.</div>';
    }
    ...
    return products
      .map(function (product) {
        return (
          ...
          " · Univers : " +
          escapeHtml(product.sourceLabel || "Commerce") +
          " · Lignes : " +
    ...
    container.innerHTML = clients
      .map(function (client) {
        return (
          ...
          "<br>Univers : " +
          escapeHtml(client.sourceLabel || "Commerce") +
          "<br>Téléphone : " +
          ...
          "</span>" +
          "</div>" +
          '<div class="client-products-title">Détail vendu</div>' +
  ...
  function getReportInputs() {
    return {
      ...
      commercialName: (document.getElementById("commercialName").value || "").trim() || "Non renseigné",
      reportTitle: (document.getElementById("reportTitle").value || "").trim() || "Rapport journalier global Auto + Industrie",
  ...
  function buildPreviewWindowClientHtml(client) {
    return (
      ...
      "</div>" +
      '<div class="preview-subtitle">Détail vendu</div>' +
  ...
  function buildPreviewRanking(items, type) {
    if (!items.length) {
      return '<div class="preview-ranking-row"><div><div class="preview-ranking-name">Aucune donnée</div><div class="preview-ranking-meta">Aucun élément disponible sur cette date.</div></div></div>';
    }
  ...
  function buildPreviewWindowHtml() {
    ...
    var executiveHtml = reportStats.executivePoints.length
      ? reportStats.executivePoints
          ...
          .join("")
      : "<li>Aucune synthèse disponible.</li>";
  ...
  function buildPdfRankingRows(items, type) {
    if (!items.length) {
      return '<tr><td colspan="3">Aucune donnée exploitable.</td></tr>';
    }
    ...
    return groupedClients
      .map(function (client) {
        ...
        return (
          ...
          "</span>" +
          "</div>" +
          '<div class="pdf-subsection">Détail vendu</div>' +
          productsHtml +
  ...
  async function chargerRapportJournalier() {
    ...
    try {
      setStatus("Chargement des visites Auto + Industrie du jour...");
  ...
  async function exporterPDF() {
    ...
    try {
      ...
      var filename = "rapport_journalier_global_" + safeDate + ".pdf";
  ...
  function initPage() {
    ...
    renderInsights(reportStats);
    renderPreview([]);
    setStatus("Prêt. Choisis une date puis charge les visites Auto + Industrie.");
  }
