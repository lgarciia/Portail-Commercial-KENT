...
(function(){
  ...
  const CONFIG = {
    ...
    BUDGET_FILES: {
    2024: "budgetford.xlsx",
    2025: "budgetford.xlsx",
    2026: "budgetford.xlsx",
    2027: "budgetford.xlsx",
    2028: "budgetford.xlsx",
    },

    REAL_FILE: "activitereelford.xlsx",

    // âœ… IMPORTANT : Ford/Direct = Date commande
    REAL_DATE_COL: "Date commande",

    EVOL_REPORT_PAGE: "evol-reporting-ford.html",
    WEEK_VIEW_PAGE: "vue_semaine.html",

    COMMENT_CACHE_PREFIX: "EVOL_ANALYSE_FORD_COMMENTS_",
  };
  ...
  async function loadBudgetForYear(year){
    const file = CONFIG.BUDGET_FILES[year] || "budgetford.xlsx";
  ...
  async function loadRealForYear(year){
    ...
    try{
      ...
      // âœ… FIX : ici on ajoute "Description produit"
      const colDes = findCol(headerMap, [
        "description produit",      // <-- TA COLONNE
        "description",
      ...
      for (const r of raw){
        ...
        rows.push({
          ...
          des: colDes ? String(r[colDes] ?? "").trim() : "",   // âœ… maintenant Ã§a remplira
    ...
    el.tbl.querySelectorAll("button[data-weekbtn='1']").forEach(btn => {
      btn.addEventListener("click", (e) => {
        ...
        const qs = new URLSearchParams({
          ...
          amountColHint: "CA Total", // Ford/Direct
          year: String(selectedYear),
      budgetFile: (CONFIG.BUDGET_FILES[selectedYear] || "budgetford.xlsx")
        });
