...
(function(){
  ...
  // CONFIG
  // =========================
  const FILE_NAME = "activitereelpsa.xlsx";

  // Ton pivot historique
  const STORAGE_KEY_PIVOT = "ACTIVITE_REEL_2026_V1";

  // âœ… ClÃ© attendue par repoting-annuelpsa.html
  const STORAGE_KEY_REPORTING_PSA = "ACTIVITEREEL_PSA_V1";
  ...
  el.btnReset.addEventListener("click", () => {
    if (!confirm("Supprimer les donnÃ©es ActivitÃ© RÃ©elle stockÃ©es en local ?")) return;
    localStorage.removeItem(STORAGE_KEY_PIVOT);
    localStorage.removeItem(STORAGE_KEY_REPORTING_PSA);
    pivot = null;
    sortTotalDir = null;
  ...
  function parseAndBuildPivot(arrayBuffer, sourceLabel){
    ...
    const colClientInterne = findCol(headerMap, ["nÂ° client interne","no client interne","n client interne","nÂ° client","numero client interne"]);
    ...
    // âœ… 1) Sauvegarde pivot (ta clÃ© historique)
    saveJSON(STORAGE_KEY_PIVOT, pivot);

    // âœ… 2) Sauvegarde format â€œreporting annuel PSAâ€ attendu
    saveActiviteReelPSAFromPivot(pivot);
  ...
  // =========================
  // âœ… BRIDGE vers reporting annuel PSA
  // =========================
  function saveActiviteReelPSAFromPivot(p){
    ...
    localStorage.setItem(STORAGE_KEY_REPORTING_PSA, JSON.stringify(payload));
    return payload;
  }

  function updateSyncLabel(){
    const payload = loadJSON(STORAGE_KEY_REPORTING_PSA, null);
  ...
  function exportActiviteReelPSAJson(){
    const payload = loadJSON(STORAGE_KEY_REPORTING_PSA, null);
    if (!payload?.clients?.length){
      alert("Aucune donnÃ©e RÃ©el PSA en cache. Recharge dâ€™abord le fichier.");
      return;
    }
    downloadBlob(
      new Blob([JSON.stringify(payload, null, 2)], { type:"application/json" }),
      `activitereel_psa_${stamp()}.json`
    );
  }
...
function fmt0(n){
  ...
  function makeReportingClientKey(clientInternal, clientName){
    const a = normalizeKey(clientInternal).replace(/[^a-z0-9]+/g,"").slice(0, 40);
    const b = normalizeKey(clientName).replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"").slice(0, 60);
