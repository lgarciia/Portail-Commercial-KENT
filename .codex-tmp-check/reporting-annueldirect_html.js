...
(async function(){
  ...
  const STORAGE = {
    budget: "BUDGET_DIRECT_DATA_V1",
    budgetMeta: "BUDGET_DIRECT_META_V1",
    real: "BUDGET_DIRECT_REAL_V1",
  };

  // âœ… sources fichiers
  const REAL_FILE = "activitereeldirect.xlsx";
  const BUDGET_FILE = "budgetdirect.xlsx";
  ...
  el.btnResetReal.addEventListener("click", () => {
    if (!confirm("Recharger toutes les valeurs RÃ©el depuis activitereeldirect.xlsx ?")) return;
  ...
  async function importBudgetFile(file){
    try{
      ...
      // âœ… fetch rÃ©el aprÃ¨s import budget
  ...
  function parseBudget(arrayBuffer, fileLabel){
    ...
    // Budget Direct : on accepte plusieurs conventions
    const colClient = findCol(headerMap, ["client", "nom client", "nom du client"]);
    const colCode = findCol(headerMap, ["code livrÃ©","code livre","code","n client","nÂ° client","no client","numero client","nclient"]);

    if (!colClient && !colCode) throw new Error("Budget : colonnes Client/Code livrÃ© introuvables.");

    const monthCols = {};
    for (const m of MONTHS){
      const col = findCol(headerMap, [normalizeKey(m.excel)]);
      if (!col) throw new Error(`Budget : colonne "${m.excel}" introuvable.`);
      monthCols[m.key] = col;
    }

    const clients = [];
    for (const r of rows){
      const name = colClient ? String(r[colClient] ?? "").trim() : "";
      const code = colCode ? String(r[colCode] ?? "").trim() : "";
      if (!name && !code) continue;

      const id = makeClientKey(name, code);

      const budget = {};
      for (const m of MONTHS) budget[m.key] = Math.round(toNumber(r[monthCols[m.key]]));

      clients.push({ id, name: name || `Client ${code}`, nclient: code, budget });
    }
  ...
  async function tryAutoFetchBudget(){
    try{
      ...
      // si OK, on charge comme si import
  ...
  /* ======================================================
     âœ… FETCH RÃ‰EL (activitereeldirect.xlsx)
     Colonnes : Code livrÃ© | Nom client | CA Total | mois2
  ...
  async function loadRealFromFetch(){
    ...
    try{
      ...
      const headerMap = buildHeaderMap(rows[0] || {});
      const colCode = findCol(headerMap, ["code livrÃ©","code livre","code"]);
      const colName = findCol(headerMap, ["nom client","nom du client","client"]);
      const colAmt  = findCol(headerMap, ["ca total","ca","chiffre d'affaires","chiffre daffaires"]);
      ...
      for (const bc of budgetData.clients){
        ...
        addLookupCandidate(budgetByCode, compactClientCode(bc.nclient), bc);
        addLookupCandidate(budgetByName, compactClientName(bc.name), bc);
      }

      function ensureBudgetClient(codeLivre, clientName){
        const knownClient = findMatchedClient(budgetByExact, budgetByCode, budgetByName, codeLivre, clientName);
        if (knownClient) return knownClient;

        const rawCode = String(codeLivre || "").trim();
        const rawName = String(clientName || "").trim();
        if (!rawCode && !rawName) return null;

        // client hors budget
        const name = rawName || `Client ${rawCode}`;
        const id = makeClientKey(name, rawCode);

        const budget = {};
        for (const m of MONTHS) budget[m.key] = 0;

        const bc = { id, name, nclient: rawCode, budget, isUnbudgeted: true };

        budgetData.clients.push(bc);
        const exactKey = makeClientMatchKey(name, rawCode);
        if (exactKey) budgetByExact.set(exactKey, bc);
        addLookupCandidate(budgetByCode, compactClientCode(rawCode), bc);
      ...
      for (const r of rows){
        ...
        const codeLivre = String(r[colCode] ?? "").trim();
        const clientName = colName ? String(r[colName] ?? "").trim() : "";

        const bc = ensureBudgetClient(codeLivre, clientName);
        if (!bc) continue;
  ...
  function monthNameToKey(m){
    ...
    const map = {
      "janvier":"jan","jan":"jan",
      "fevrier":"feb","fÃ©vrier":"feb","fev":"feb","fÃ©v":"feb",
      "mars":"mar",
      "avril":"apr","avr":"apr",
      "mai":"may",
      "juin":"jun",
      ...
      "aout":"aug","aoÃ»t":"aug",
      "septembre":"sep","sep":"sep","sept":"sep",
      "octobre":"oct","oct":"oct",
      "novembre":"nov","nov":"nov",
      "decembre":"dec","dÃ©cembre":"dec","dec":"dec","dÃ©c":"dec"
    };
    return map[k] || null;
  }
  ...
  /* ======================================================
     RENDER (mÃªme structure que PSA : RÃ©el/Budget/Ã‰cart cÃ´te Ã  cÃ´te)
  ====================================================== */
    ...
    el.table.innerHTML = `
      <thead>
        <tr>
          ...
          <th class="col-nclient">Code livrÃ©</th>
  ...
  function render(){
    ...
    const clients = budgetData.clients
      .filter(c =>
        ...
        String(c.nclient||"").toLowerCase().includes(filterText)
      );
  ...
  function buildExportRows(){
    ...
    function rowFromCumul(c){
      const r = { CLIENT: "Cumul", "CODE LIVRÃ‰": "" };
      for (const m of MONTHS){
    ...
    function rowFromClient(c){
      const r = { CLIENT: c.name, "CODE LIVRÃ‰": c.nclient || "" };
  ...
  function exportJSON(){
    ...
    downloadBlob(
      ...
      `reporting_annuel_direct_${stamp()}.json`
    );
  }
  ...
  function exportCSV(){
    ...
    downloadBlob(
      ...
      `reporting_annuel_direct_${stamp()}.csv`
    );
  }
  ...
  function exportXLSX(){
    ...
    const ws = XLSX.utils.json_to_sheet(rows);
    XLSX.utils.book_append_sheet(wb, ws, "Reporting annuel DIRECT");
    const out = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    downloadBlob(
      new Blob([out], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }),
      `reporting_annuel_direct_${stamp()}.xlsx`
    );
  }
  ...
  function compactClientCode(value){
    return normKey(value).replace(/[^a-z0-9]+/g,"").slice(0, 60);
  }
  ...
  function makeClientMatchKey(name, nclient){
    ...
    if (nc && nm) return `code_${nc}__${nm}`;
    if (nc) return `code_${nc}`;
