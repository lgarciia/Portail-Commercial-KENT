...
(function(){
  ...
  const CONFIG = {
    ...
    // Budget (tu peux mettre 1 fichier unique ou par annÃ©e)
    BUDGET_FILES: {
    2024: "budgetpsa.xlsx",
    2025: "budgetpsa.xlsx",
    2026: "budgetpsa.xlsx",
    ...
    // RÃ©el PSA (fetch)
    REAL_FILE: "activitereelpsa.xlsx",

    // âœ… colonne date pour la vue semaine (si elle existe dans ton rÃ©el)
    REAL_DATE_COL: "Date vente",

    // Page Evol Report vs Budget
    EVOL_REPORT_PAGE: "evol-reporting-psa.html",

    // âœ… Page vue semaine
    WEEK_VIEW_PAGE: "vue_semaine.html",

    // Cache commentaires
    COMMENT_CACHE_PREFIX: "EVOL_ANALYSE_PSA_COMMENTS_",
  };
  ...
  // lignes rÃ©el pour drilldown (clients/achats + vue semaine)
  // normalized rows: {monthIdx, weekIdx, date, clientInternal, clientName, amount, ref, des, qty}
  let realRows = [];
  ...
  /* =========================
     BUDGET: fetch + somme mensuelle globale
  ========================= */
  async function loadBudgetForYear(year){
    const file = CONFIG.BUDGET_FILES[year] || "budgetpsa.xlsx";
  ...
  /* =========================
     REAL: fetch + somme mensuelle globale + stockage drilldown
     âœ… + weekIdx calculÃ© via Date vente si dispo
  ========================= */
  async function loadRealForYear(year){
    ...
    try{
      ...
      // âœ… date vente (pour semaines)
      const colDate = findCol(headerMap, [CONFIG.REAL_DATE_COL, "date vente", "date"]);
      // mois fallback
      ...
      const colClientInt = findCol(headerMap, [
        "nÂ° client interne","no client interne","n client interne","numero client interne","nÂ° client","code livrÃ©","code livre","code"
      ]);
      const colClientName = findCol(headerMap, ["nom du client","nom client","client"]);

      // dÃ©tail achats (si dispo)
      const colRef = findCol(headerMap, ["reference produits","rÃ©fÃ©rence produits","reference produit","rÃ©fÃ©rence produit","ref produit","ref"]);
      const colDes = findCol(headerMap, ["dÃ©signation","designation","design"]);
      ...
      for (const r of raw){
        ...
        rows.push({
          monthIdx: mi,
          weekIdx: dt ? weekBucket(dt) : -1, // 0..3 si date ok
          date: dt,
      ...
      const dateInfo = colDate ? `Date OK (${CONFIG.REAL_DATE_COL})` : `Sans Date vente (mode mois2)`;
      el.realText.textContent = `RÃ©alisÃ© : OK (${file}) â€” sheet: ${sheetName} â€” ${dateInfo}`;
  ...
  /* =========================
     TABLE main
     âœ… Ajout bouton â€œSem.â€ par mois
  ========================= */
  function renderTable(){
    ...
    // âœ… click "Sem." -> page semaine (sans dÃ©clencher le drilldown)
    el.tbl.querySelectorAll("button[data-weekbtn='1']").forEach(btn => {
      btn.addEventListener("click", (e) => {
        ...
        // month key attendu par vue_semaine : jan/feb/... (comme tes autres pages)
        ...
        const qs = new URLSearchParams({
          ...
          // budget : on passe le fichier + annÃ©e (la page semaine refetch si tu veux)
          // ðŸ‘‰ si tu prÃ©fÃ¨res localStorage (comme evol-reporting), dis-moi et je te le cÃ¢ble pareil.
          year: String(selectedYear),
      budgetFile: (CONFIG.BUDGET_FILES[selectedYear] || "budgetpsa.xlsx")
        });
    ...
    // bind month click -> drilldown clients
  ...
  /* =========================
     Drilldown: Client -> Purchases (month)
  ========================= */
  function openClientDetails(monthIdx, clientInternal, clientName){
    ...
    if (hasProduct){
      ...
      blocksHtml = `
        <div class="empty">Pas de colonnes RÃ©f/DÃ©signation dÃ©tectÃ©es â†’ affichage brut des lignes.</div>
        ${rows.slice(0, 300).map(r => `
          <div class="listCard" style="cursor:default;">
            ...
            <div class="listSub">
              <span class="tag">NÂ°: <b>${esc(r.clientInternal || "â€”")}</b></span>
              <span class="tag">Client: <b>${esc(r.clientName || "â€”")}</b></span>
            </div>
          </div>
        `).join("")}
      `;
  ...
  function bindDrawer(){
    ...
    window.addEventListener("pageshow", () => closeDrawer()); // bfcache safe
  }
  ...
  function saveComments(year){
    try{
      localStorage.setItem(commentsKey(year), JSON.stringify(comments || {}));
    }catch{}
  }
  ...
  // âœ… 4 semaines fixes : 1..7 / 8..14 / 15..21 / 22..fin
