...
(async function(){
  ...
  el.sBudget.textContent = "chargementâ€¦";
  el.sReel.textContent = "chargementâ€¦";
  ...
  function generate(){
    if (!budgetData?.clients?.length){
      el.resultTable.innerHTML = emptyTable("Aucune donnÃ©e Budget (fetch XLSX). VÃ©rifie le fichier / nom / chemin.");
      setKpis(0,0,0,0);
      lastExportRows = [];
      return;
    }
  ...
  function parseBudgetWorkbook(wb){
    ...
    for (let i=0;i<raw.length;i++){
      ...
      const id = makeClientMatchKey(name, nclient) || ("row_" + i);
