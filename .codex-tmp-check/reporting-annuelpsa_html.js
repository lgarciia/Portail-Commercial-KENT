...
(async function(){
  ...
  const BUDGET_FILE = "budgetpsa.xlsx";
  ...
  el.btnResetReal.addEventListener("click", () => {
    if (!confirm("Recharger toutes les valeurs RÃ©el depuis activitereelpsa.xlsx ?")) return;
  ...
  async function importBudgetFile(file){
    try{
      ...
      // âœ… NOUVEAU : recharge rÃ©el fetch (Ã©crase)
  ...
  /* ======================================================
     âœ… NOUVEAU : FETCH RÃ‰EL (activitereelpsa.xlsx)
  ...
  async function loadRealFromFetch(){
    ...
    try{
      ...
      // colonnes attendues (tolÃ©rance accents/casse)
      const headerMap = buildHeaderMap(rows[0] || {});
...
function ensureBudgetClient(nclientInterne, clientName){
  ...
  // âœ… Sinon : on crÃ©e un "client hors budget"
  ...
  return bc;
      ...
      for (const r of rows){
        ...
        // matching prioritaire : N CLIENT (budget) vs NÂ° client interne (rÃ©el)
...
if (!bc) continue;
      ...
      // on ne dÃ©truit pas le cache realData existant en cas d'erreur
    }
  }
