...
(async function(){
  ...
  const STORAGE = {
    budget: "BUDGET_FORD_DATA_V1",
    budgetMeta: "BUDGET_FORD_META_V1",
    real: "BUDGET_FORD_REAL_V1",
  };

  // âœ… sources fichiers
  const REAL_FILE = "activitereelford.xlsx";
  ...
  function parseBudget(arrayBuffer, fileLabel){
    ...
    // Budget Ford : on accepte plusieurs conventions
  ...
  /* ======================================================
     âœ… FETCH RÃ‰EL (activitereelford.xlsx)
  ...
  function exportJSON(){
    ...
    downloadBlob(
      ...
      `reporting_annuel_ford_${stamp()}.json`
    );
  }
  ...
  function exportCSV(){
    ...
    downloadBlob(
      ...
      `reporting_annuel_ford_${stamp()}.csv`
    );
  }
  ...
  function exportXLSX(){
    ...
    const ws = XLSX.utils.json_to_sheet(rows);
    XLSX.utils.book_append_sheet(wb, ws, "Reporting annuel FORD");
    const out = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    downloadBlob(
      new Blob([out], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }),
      `reporting_annuel_ford_${stamp()}.xlsx`
    );
  }
