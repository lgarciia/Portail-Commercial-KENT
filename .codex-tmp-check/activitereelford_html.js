...
(function(){
  ...
  // CONFIG
  // =========================
  const FILE_NAME = "activitereelford.xlsx"; // â† mets le nom exact de ton fichier si diffÃ©rent

  // Pivot historique
  const STORAGE_KEY_PIVOT = "ACTIVITE_REEL_FORD_2026_V1";

  // âœ… ClÃ© attendue par ton reporting annuel Ford
  const STORAGE_KEY_REPORTING_FORD = "ACTIVITEREEL_FORD_V1";
  ...
  el.btnReset.addEventListener("click", () => {
    if (!confirm("Supprimer les donnÃ©es ActivitÃ© RÃ©elle Ford stockÃ©es en local ?")) return;
    localStorage.removeItem(STORAGE_KEY_PIVOT);
    localStorage.removeItem(STORAGE_KEY_REPORTING_FORD);
    pivot = null;
    sortTotalDir = null;
  ...
  function parseAndBuildPivot(arrayBuffer, sourceLabel){
    ...
    // âœ… Bridge vers reporting annuel FORD (format identique PSA)
    saveActiviteReelFordFromPivot(pivot);
  ...
  // =========================
  // âœ… BRIDGE vers reporting annuel Ford
  // =========================
  function saveActiviteReelFordFromPivot(p){
    ...
    localStorage.setItem(STORAGE_KEY_REPORTING_FORD, JSON.stringify(payload));
    return payload;
  }

  function updateSyncLabel(){
    const payload = loadJSON(STORAGE_KEY_REPORTING_FORD, null);
  ...
  function exportJSON(){
    ...
    downloadBlob(
      ...
      `activite_reelle_ford_${stamp()}.json`
    );
  }
  ...
  function exportCSV(){
    ...
    downloadBlob(new Blob([csv], { type: "text/csv;charset=utf-8" }), `activite_reelle_ford_${stamp()}.csv`);
  }
  ...
  function exportXLSX(){
    ...
    downloadBlob(
      ...
      `activite_reelle_ford_${stamp()}.xlsx`
    );
  }
